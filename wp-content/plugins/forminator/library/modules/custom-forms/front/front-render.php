...
class Forminator_CForm_Front extends Forminator_Render_Form {
	...
	public function display( $id, $ajax = false, $data = false, $hide = true ) {
		...
		if ( $this->model instanceof Forminator_Custom_Form_Model && ( $ajax || Forminator_Custom_Form_Model::STATUS_PUBLISH === $this->model->status ) ) {
		...
		foreach ( $fonts as $setting_name => $font_name ) {
			if ( ! empty( $font_name ) ) {
				...
				if ( ! $ajax ) {
					...
					?>
					<link rel="stylesheet" <?php //phpcs:ignore ?>
					      id="forminator-font-<?php echo sanitize_title( $font_name ); ?>-css" <?php //phpcs:ignore ?>
					      href="<?php echo $google_font_url; ?>" <?php //phpcs:ignore ?>
					      type="text/css"
					      media="all"/>
					<?php
				}
	...
	public function pagination_submit_button() {
		...
		if ( $this->get_form_design() !== 'material' ) {
			...
			$html
				=
				sprintf( '<button class="' . $class . '" style="display: none;" disabled><span class="forminator-button--mask" aria-label="hidden"></span><span class="forminator-button--text">%s</span></button>',
					$button );
		}
...
//		$html .= $this->get_description( $field );
	...
	public function get_button_markup() {
		...
		if ( $this->get_form_design() !== 'material' ) {
			...
			$html .= sprintf( '<button id="forminator-submit" class="' . $class . '"><span class="forminator-button--mask" aria-label="hidden"></span><span class="forminator-button--text">%s</span></button>',
				$button );
		}
		$html .= '</div>';
		$html .= '</div>';
