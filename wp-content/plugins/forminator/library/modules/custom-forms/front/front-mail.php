...
class Forminator_CForm_Front_Mail extends Forminator_Mail {
	...
	public function process_mail( $custom_form, $data, Forminator_Form_Entry_Model $entry ) {
		...
		if ( $this->send_admin_mail( $setting ) || $this->send_user_mail( $setting ) ) {
			...
			if ( $this->send_admin_mail( $setting ) ) {
				$recipients = $this->get_admin_email_recipients( $setting );
				 ...
				 */
				$recipients = apply_filters( 'forminator_custom_form_mail_admin_recipients', $recipients, $custom_form, $data, $entry, $this );
	 ...
	 * @return array
	 */
	public function get_admin_email_recipients( $setting ) {
		$email = array();
		...
		$email = apply_filters_deprecated( 'forminator_get_admin_email_recipents', array( $email, $setting ), '1.6', 'forminator_get_admin_email_recipients' );

		return apply_filters( 'forminator_get_admin_email_recipients', $email, $setting );
	}
